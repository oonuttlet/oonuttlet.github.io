# function wrapped by `get_acs_all_years()`
# basic input for state, county, year, variables
...
# some incidents can overlap, so this ensures that the same data is only obtained once
...
# iterate `map_data()` over returned object (state, county, variables, year) from `get_distinct_acs_info()`
