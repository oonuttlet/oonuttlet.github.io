...
```{r, message=FALSE}
# Example of tigris:
nm <- tigris::states(progress_bar = F) |>
...
nm_fires <- arcgislayers::arc_read(r"(https://services3.arcgis.com/T4QMspbfLg3qTGWY/ArcGIS/rest/services/InterAgencyFirePerimeterHistory_All_Years_View/FeatureServer/0)",
  ...
  slice_max(order_by = Shape__Area,
            n = 10) |>
...
#| fig.align: center
#| message: false
nm_fires_impact <- get_incident_impacts(data = nm_fires, 
                                        id_col = OBJECTID, 
                                        year = FIRE_YEAR)
...
ggplot(filter(nm_fires_impact, str_starts(variable, "pop"))) +
  ...
  scale_color_discrete("Race/Ethnicity Category", labels = c("Hispanic/Latino",
...
```{r}
#| message: false
nm_fires_impact_wide <- get_incident_impacts(data = nm_fires,
                                             id_col = OBJECTID,
                                             year = FIRE_YEAR,
                                             output = "wide")
